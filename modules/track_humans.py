...
class TrackHumans:
    ...
    def track_humans(self, frame, humans):
        ...
        if frame == self.start:
            ...
            self.humans_current = np.concatenate((humans_current, np.c_[self.humans_id]), axis=1)
            self.humans_tracklet = (np.concatenate((self.humans_tracklet[self.humans_tracklet[:, 0] > (frame - 32)],
    ...
    def search_nearest(humans, prev_humans, prev_id):
        ...
        new_appearance = np.where(nearest_body_dist[0, nearest_body_num] > 500)[0]
